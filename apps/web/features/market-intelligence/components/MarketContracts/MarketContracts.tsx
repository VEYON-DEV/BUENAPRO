...
export function MarketContracts({ rows, total, filters }: { rows: any[]; total: number; filters: any }) {
  ...
  return (
    <section className={styles.section}>
      ...
      {rows.length ? <div className={styles.scroll}><table><thead><tr><th>Contrato</th><th>Objeto</th><th>Entidad y región</th><th>Resultado</th><th>Ganador</th><th>Precio</th><th>Publicación</th></tr></thead><tbody>
        {rows.map((row) => <tr key={row.id_contrato}>
          ...
          <td><strong>{formatMoney(row.precio_total, "-")}</strong></td>
      ...
      </tbody></table></div> : <p className={styles.empty}>No encontramos contratos con estos filtros.</p>}
